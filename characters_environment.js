...
if(noseX !="" && gameConfig.status==="start" && gameStatus=="start") { document.getElementById("status").innerHTML = "Game Is Loaded";
    world_start.play();
...
function manualControl(character){
  ...
  if(character.live){
    ...
    if(!noseX &&!keyDown(control.right)&&!keyDown(control.up)){ 
...
function StepOnEnemy(obj1,obj2){
	...
	if(obj1_Right>=obj2_Left&&obj1_Left<=obj2_Right && obj1_Down<=obj2_Up+7 && obj1_Down>=obj2_Up-7 && obj2.live==true && obj2.touching.top){
		obj2.live=false;
    obj1.killing=30; mario_kick.play();
    obj1.kills++;
